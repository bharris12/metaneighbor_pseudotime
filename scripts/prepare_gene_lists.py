...
if 'snakemake' in globals():
	...
	parser.add_argument('--design-fn',type=str,required=True,help='File name for design matrix, must be genes x terms')
	parser.add_argument('--output-dir',type=str,required=True,help='Directory to output the saved gene lists and terms x filenames file to')
	parser.add_argument('--min-size',type=int,default=10, help='Minimum size of gene list')
	parser.add_argument('--max-size',type=int,default=10, help='Maximum size of gene list')
...
design = design.loc[:, (term_sizes>args.min_size) & (term_sizes<args.max_size)]

assert design.shape[1] > 0, f'No design are larger than {args.min_size} and smaller than {args.max_size}'
