...
if 'snakemake' in globals():
    ...
    parser.add_argument('--term',
                            type=str,
                            help='Name of GO term or Gene list to compute MN on or filename with genes',
                            default='HVG')
    parser.add_argument('--term-name',
                        type=str,
    ...
    parser.add_argument(
        ...
        action='store_false',
        default=True,
        help='Whether to run in Original Slow mode, default fast approximate version')
...
if Path(term).is_file():
    ...
    term_genes = adata.var_names[pymn.variableGenes(adata, study_col,return_vect=True)]

assert term_genes.shape[0] > 10 ,'Must use a term with at least 10 genes'
assert term_genes.shape[0] < max_genes or term == 'HVG', f'Must use a term with less than {max_genes}'
